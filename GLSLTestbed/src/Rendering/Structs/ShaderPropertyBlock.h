...
class ShaderPropertyBlock : public PropertyBlock
{
    public:
		...
		void SetComputeBuffer(uint32_t hashId, const GraphicsID* bufferIds, uint32_t count = 1) { SetValue(hashId, CG_TYPE_COMPUTE_BUFFER, bufferIds, count); }
		...
		void SetComputeBuffer(uint32_t hashId, GraphicsID bufferId) { SetValue(hashId, CG_TYPE_COMPUTE_BUFFER, &bufferId); }
