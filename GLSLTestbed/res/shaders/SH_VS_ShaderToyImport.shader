...
vec4 map(vec3 p)
{
	...
	return vec4(1.47 - abs(cos(p.x*0.3 - length(p.xy))*p.y) - dist, vec3(step(dist, 0.1))); // planes
}
...
{
	iTime = pk_Time.y;
	iResolution = vec3(pk_ScreenParams.xy, 0.0);
	mainImage(SV_Target0, vs_TEXCOORD0.xy * pk_ScreenParams.xy);
	SV_Target0.a = 1.0;
}